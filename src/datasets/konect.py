...
class KonectDataset(InMemoryDataset):
    url = 'https://github.com/unai-zulaika/LWP-WL-Pytorch/raw/master/lwp-wl/data/raw'
    ...
    def process(self):
        ...
        if self.name.lower() != 'usair':
            data[:, 2] = np.exp(-1 / data[:, 2])
        num_nodes = int(data[:, 0:2].max()) + 1
        data = Data(
            edge_index=torch.tensor(data[:, 0:2].T, dtype=torch.long),
